...
frappe.ui.form.on('Barcode Printing', {
	...
	refresh: function(frm) {
		frm.add_custom_button(__('Purchase Receipt'), function() {
			erpnext.utils.map_current_doc({
				method: "techstation_item_control.techstation_item_control.doctype.barcode_printing.barcode_printing.pr_make_barcode",
		...
		frm.add_custom_button(__('Stock Entry'), function() {
			erpnext.utils.map_current_doc({
				method: "techstation_item_control.techstation_item_control.doctype.barcode_printing.barcode_printing.se_make_barcode",
		...
		frm.add_custom_button(__('Stock Reconciliation'), function() {
			erpnext.utils.map_current_doc({
				method: "techstation_item_control.techstation_item_control.doctype.barcode_printing.barcode_printing.sr_make_barcode",
				source_doctype: "Stock Reconciliation",
				target: frm,
				date_field: "posting_date",
				setters: {
					// purpose: frm.doc.purpose || undefined,
				},
				get_query_filters: {
					docstatus: 1
				}
	...
	get_items: function(frm) {
		let fields = [
			...
			// },
			{
				...
				options: 'Company',
				reqd: 1,
				default:frappe.defaults.get_global_default("company") || frappe.defaults.get_user_default("company")
			},
			{
		...
		frappe.prompt(fields, function(data) {
			frappe.call({
				method: "techstation_item_control.techstation_item_control.doctype.barcode_printing.barcode_printing.get_items",
				args: {
		...
		frm.doc.items.forEach(d =>			
			{
				if (d.item_code)
				{
					console.log("HELLO");
					frappe.call({
						method: "techstation_item_control.techstation_item_control.doctype.barcode_printing.barcode_printing.search_item_serial_or_batch_or_barcode_number",
						args: {
							search_value:1,
							item:d
						},
	...
	{
		frappe.call({
			method: "techstation_item_control.techstation_item_control.doctype.barcode_printing.barcode_printing.make_qrcode",
			args: {
				doc:frm.doc,
